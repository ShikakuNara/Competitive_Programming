...
int main()
{
    ...
    rep(tt,t)
    {
      ...
      while(cnt<n)
      {
        ...
        {
          ll u=d[i].Y;
          if(po[u]<=xx*ang-180+xx&&po[u]>=xx*ang-180&&vis[u]==0)
          if(cur+Max(vol[gf[u].X],vol[gf[u].Y])<=b)cur+=Max(vol[gf[u].X],vol[gf[u].Y]),ii.pb(u);
        }
        reverse(ii.begin(),ii.end());
        //find children on the way with the bags we need
        bool ff=0;
        rep(i,0,n-1)
        {
          ll u=d[i].Y;
          rep(j,sz(ii))if(ii[j]==u){ff=1;break;}
          if(ff)continue;
          ...
          ll ind=-1;
          ff=1;
          rep(j,sz(ii))if(gf[u].X==gf[ii[i]].Y&&dis[u]<dis[ii[i]]){ind=ii[i];break;}
          if(ind==-1)continue;
